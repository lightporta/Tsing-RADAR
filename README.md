...
```bash
# 后端单元 + 集成测试（882 passed / 4 failed（Windows 环境性）/ 2 skipped / 2 errors；
# v4.2.1 新增 19 验收用例：访谈引擎 8 项修复（幽灵硬约束/澄清死循环/回声环/
# SSE 收尾/增强降级/画像清洗/发问去重/无候选兜底，见 docs/访谈引擎修复方案验收记录_v421.md）；
# v4.2.2 新增 26 回归 + 5 表达层调和用例：单选句子降级/off_topic 逃生/英文解析/
# P0-J 路由回退/放宽重跑/兜底卡一次/SSE 非流式开关/增强链路调和（模板 v4 选项逐字）；

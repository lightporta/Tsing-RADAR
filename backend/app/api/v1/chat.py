...
from app.services.off_topic import (
    detect_off_topic_matched,
    is_acknowledgment,
    is_farewell,
    is_greeting,
)
...
from app.services.mentor_knowledge import (
    query_mentor_knowledge,
    render_mentor_knowledge,
    render_mentor_not_found,
)
...
# v4.2.x 修复5：四个单选维度的答案由确定性状态机直接产出，跳过 LLM 增强
# 即消除 100-150s 串行等待；自由文本题（研究兴趣/硬约束）保留增强。
_SINGLE_CHOICE_DIMENSIONS = frozenset(
    {"research_mode", "mentorship_style", "career_orientation", "innovation_risk"}
)


def _answered_question_is_single_choice(
    db: Session,
    *,
    session_id: str,
    student_id: str | None,
) -> bool:
    """v4.2.x 修复5：本轮被回答的题目是否为单选（单选轮跳过 LLM 增强）。

    sync_user_transcript/answer_session 推进状态机后 current_question_id 已
    指向下一题，无法再回看，因此必须在推进前读取被回答的题目；会话不存在
    （首轮自由文本题）→ False。确认门/匹配结果（needs_confirmation /
    recommend_ready）的跳过由调用处既有红线处理，不受本函数影响。
    """
    try:
        pre = get_session(db, session_id, student_id)
    except InterviewNotFoundError:
        return False
    return (pre.current_question_id or "") in _SINGLE_CHOICE_DIMENSIONS


# v4.2.x 修复8：无候选兜底指令 ——「放宽」忽略硬性条件重跑；「换方向：XXX」
# 单次主题覆盖重跑。均为 recommend_ready 上下文指令，不影响访谈流程。
_RELAX_HARD_CONSTRAINTS_TRIGGERS = (
    "放宽",
    "放宽条件",
    "忽略硬约束",
    "忽略硬性条件",
...
_TOPIC_OVERRIDE_RE = re.compile(r"换方向\s*[:：]?\s*(.+)$")
_NO_CANDIDATE_CARD_FLAG = "no_candidate_card_shown"
_NO_CANDIDATE_FALLBACK_CARD = (
    "当前没有同时通过硬约束与召回阈值的候选。你可以：\n"
    "- 回复「放宽」：忽略硬性条件，重新匹配一次\n"
    "- 回复「换方向：XXX」：按新主题重试（如「换方向：大模型」）\n"
    f"- 浏览完整导师目录：{_SITE_HOME_URL}"
)


def _relax_trigger_match(text: str) -> bool:
    return any(term in text for term in _RELAX_HARD_CONSTRAINTS_TRIGGERS)


def _parse_topic_override(text: str) -> list[str] | None:
    """「换方向：大模型、推荐系统」→ 清洗后的主题标签；非指令返回 None。"""
    match = _TOPIC_OVERRIDE_RE.search(text.strip())
    if not match:
        return None
    tags = _interest_tags(match.group(1))
    return tags or None


def _fallback_command(text: str) -> bool:
    """确认指令 / 放宽 / 换方向：这些指令在空结果时不算跑题，须走兜底。"""
    return (
        text.strip() in _CONFIRM_SIGNALS
        or _relax_trigger_match(text)
        or _parse_topic_override(text) is not None
    )


def _append_no_candidate_card(
    db: Session, *, session_id: str, student_id: str | None, visible: str
) -> str:
    """v4.2.x 修复8：空候选追加一次性兜底引导（会话级去重，不刷屏）。"""
    if has_session_flag(
        db,
...
# —— v4.2.2（P0-J）：推荐后"路由黑洞"治理 ——
# 非结构指令消息不再无条件重跑匹配，先走确定性通用问答回退：
# 告别 / 导师知识库（含无"老师/教授"后缀的松散问法）/ 平台机制 FAQ /
# 结果指引 / 能力引导。识别不了的闲聊诚实拒答，绝不编造（P1-D）。
_FAREWELL_REPLY = (
    "好的，再见～之后想继续匹配、查看候选详情、写简历或了解招募信息，"
    "随时回来找我。"
)
_ACK_REPLY = (
    "不客气～匹配结果已在上方，可以继续追问「第 N 个」查看详情、"
    "「雷达图」查看证据雷达，或回复「换一批」「缩小范围」调整结果。"
)
_GREETING_REPLY = (
    "你好～匹配结果已在上方，可以继续追问「第 N 个」查看详情、"
    "「雷达图」查看证据雷达，或回复「换方向：XXX」换个主题重试。"
)
_RESULT_POINTER_REPLY = (
    "匹配结果已在上方，可以继续追问「第 N 个」查看详情、"
    "「雷达图」查看证据雷达，或回复「换一批」「缩小范围」调整结果。"
)
# v4.2.2（P0-J 细化）：显式"查看/展示结果"指令与匹配相关追问仍放行走既有
# 匹配重跑展示（与 v3.1.6 契约一致），避免结果指引/能力引导吞掉真实诉求。
_VIEW_RESULT_VERBS = (
    "查看", "看看", "看下", "展示", "显示", "给我", "刷新", "再看",
)
_MATCH_RELEVANT_ASK_TERMS = (
    "更适合我", "适合我", "推荐我", "帮我选", "哪个导师", "哪个更",
    "怎么选", "选哪个", "推荐哪个", "哪个适合", "谁适合",
)


def _wants_match_results_rerun(text: str) -> bool:
    """推荐后上下文里仍应触发匹配重跑的消息（而非结果指引/能力引导）。"""
    if any(verb in text for verb in _VIEW_RESULT_VERBS) and any(
        term in text for term in ("结果", "候选", "匹配")
    ):
        return True
    return any(term in text for term in _MATCH_RELEVANT_ASK_TERMS)
_COMPOUND_CMD_GUIDE = (
    "我注意到这条消息里包含多个操作指令（如「换方向」「改偏好」同时出现）。"
    "为避免记错，请一次只发一条指令，例如「换方向：电池安全」。\n"
    "修改偏好请在画像卡中编辑后重新确认；换方向/放宽在确认画像后执行。"
)
# 松散导师问法（推荐后上下文）：无"老师/教授"后缀，如"邓东灵的研究方向
# 是什么"；姓氏+停用词过滤复用 dialogue_intent 的确定性词表，防误伤。
_LOOSE_MENTOR_QUERY_RE = re.compile(
    r"^([\u4e00-\u9fa5]{2,4}?)(?:老师|教授|导师)?的?(?:研究方向|研究什么|"
    r"做什么|带学生|招生|课题组|评价|怎么样|如何|联系方式|邮箱|主页|"
    r"名额|口碑|风评|缺点)"
)
# 推荐后上下文仍允许的画像修改表达（"把我的指导偏好改成自主"）：
# 这类消息放行走既有匹配流程（画像在 sync 阶段已更新，重跑展示新结果）。
_PORTRAIT_EDIT_RE = re.compile(
    r"(?:改(?:成|为)?\s*(?:我的)?(?:研究兴趣|研究方向|指导|偏好|生涯|风险|"
    r"画像|硬性条件))"
    r"|(?:把\s*.{0,15}?(?:改成|改为))"
    r"|(?:修改\s*(?:画像|偏好|方向|兴趣))"
)
...
def _loose_mentor_name(text: str) -> str | None:
    stripped = (text or "").strip().rstrip("。！？!?？")
    match = _LOOSE_MENTOR_QUERY_RE.match(stripped)
    if not match:
        return None
    name = match.group(1)
...
def _platform_faq_answer(text: str) -> str | None:
    """平台机制类问答（契合度算法 / 数据来源 / 雷达图等），未命中返回 None。"""
...
def _compound_clauses(text: str) -> list[str]:
    return [part.strip() for part in re.split(r"[，,、；;]+", text) if part.strip()]


def _is_compound_command(text: str) -> bool:
    """多条指令混在一条消息里（逗号/顿号分隔）→ 逐条路由，一次只执行一条。

    规则：含 ≥2 个结构分句，或 1 个结构分句 + 1 个非致谢/告别的普通分句。
    "第 1 个，谢谢"（1 结构 + 1 致谢）不算复合，放行走既有流程。
    """
    clauses = _compound_clauses(text)
    if len(clauses) < 2:
        return False
    structural = 0
    soft = 0
    for clause in clauses:
        if (
            _fallback_command(clause)
            or _parse_ordinal(clause) is not None
            or _contains_structural_command(clause)
            or any(
                term in clause
                for term in (
                    _RADAR_INTENTS
            ...
            or any(term in clause for term in _REFINE_TRIGGERS)
        ):
            structural += 1
        elif is_acknowledgment(clause) or is_farewell(clause) or is_greeting(clause):
            soft += 1
    return structural >= 2 or (structural >= 1 and structural + soft < len(clauses))


def _post_match_general_query(
    *,
    latest_user: str,
    student_id: str | None,
    db: Session,
) -> str | None:
    """推荐后黑洞态通用问答回退；均未命中返回 None（走原匹配流程）。"""
    text = (latest_user or "").strip()
    if not text:
        return None
    if _PORTRAIT_EDIT_RE.search(text):
        return None  # 画像修改放行走既有流程（sync 已更新画像，需重跑展示）
    if text == _REPORT_DELIVERY_CONFIRMATION or any(
        intent in text for intent in _REPORT_INTENTS + _RADAR_INTENTS
    ):
        return None  # 报告交付确认 / 报告 / 雷达指令放行走既有分支
    if is_farewell(text):
    ...
    name = _loose_mentor_name(text)
    if name:
        record = query_mentor_knowledge(name)
        if record is None:
            return render_mentor_not_found(name)
        return render_mentor_knowledge(record)
    platform = _platform_faq_answer(text)
    if platform is not None:
        return platform
    if _wants_match_results_rerun(text):
        return None  # 显式查看结果 / 匹配相关追问 → 放行走既有匹配流程
    if any(term in text for term in ("结果", "候选", "匹配")):
        return _RESULT_POINTER_REPLY
    # 诚实兜底（P1-D）：识别不了的闲聊给能力引导，绝不编造回答/静默重跑
    return _MATCHED_OFF_TOPIC_REPLY
...
# —— v4.3.0 全 Agent 化编排接线（agent_orchestrator）——


def _request_has_media_parts(request: QXDChatRequest) -> bool:
    """用户输入是否含图片/音频/文件等多模态部分。

    媒体附件请求属确定性保留区（_prepare_llm_messages 已把媒体转为
    确定性文本占位符），一律不进 Agent 编排。
    """
...
def _agent_hook_active(
    request: QXDChatRequest,
    pending_attachments: list[SodaAttachment],
) -> bool:
    """Agent 钩子激活条件：LLM 凭据 + 编排总开关 + 无媒体附件。

    probe 请求由调用方前置排除（绝不发 LLM 请求）；输入侧多模态部分
    与本轮已生成的确定性交付附件（雷达图/套磁邮件，见 pending_attachments
    填充处）均属确定性保留区，不进 Agent。
    """
    return (
        bool(settings.llm_credentials)
        and settings.AGENT_ORCHESTRATION_ENABLED
        and not pending_attachments
        and not _request_has_media_parts(request)
    )
...
def _post_match_cheap_reply(text: str) -> str | None:
    """推荐后上下文的廉价确定性短回复（告别/感谢/问候）；未命中返回 None。

    顺序对齐 _post_match_general_query：画像修改放行优先于短回复
    （命中画像修改时返回 None，由调用方放行既有匹配流程）。
    """
    stripped = (text or "").strip()
    if not stripped:
        return None
    if _PORTRAIT_EDIT_RE.search(stripped):
        return None
    if is_farewell(stripped):
        return _FAREWELL_REPLY
    if is_acknowledgment(stripped):
        return _ACK_REPLY
...
def _post_match_rerun_release(text: str) -> bool:
    """推荐后须放行既有匹配流程的消息（画像修改 / 显式结果重跑追问）。"""
    stripped = (text or "").strip()
    if _PORTRAIT_EDIT_RE.search(stripped):
        return True
    return _wants_match_results_rerun(stripped)
...
async def generate_agent_reply(
    ...
    try:
        ...
        try:
            ...
            # v4.2.x 修复5：推进状态机前记录本轮被回答的题目是否单选
            # （sync/answer 后 current_question_id 指向下一题，无法回看）。
            answered_single_choice = _answered_question_is_single_choice(
                db, session_id=session_id, student_id=student_id
            )
            ...
            if not probe and not state.recommend_ready:
                ...
                # v4.3.0：招募注入逻辑原样保留，Agent / 非 Agent 两条
                # 回复渲染路径共用；注入条件与改动前一致（确认门与
                # 单选轮不注入）。
                recruitment_brief = ""
                if not state.needs_confirmation and not answered_single_choice:
                    interests = list(
                ...
                if _agent_hook_active(request, pending_attachments):
                    # v4.3.0 接线点 A（全 Agent 化编排）：状态机已在上方
                    # 推进并持久化，此处只接管回复渲染 —— 把事实包渲染成
                    # 状态注入文本交 run_agent_turn 编排；成功用 Agent
                    # 回复，任何失败保持状态机确定性文本（不再调用
                    # render_interview_reply 二次兜底，避免双倍 LLM 延迟）。
                    agent_fact_pack = build_interview_fact_pack(
                        state,
                    ...
                    agent_result = await run_agent_turn(
                        ...
                        student_id=student_id,
                        messages=messages,
                        state_context=_agent_interview_state_context(
                            state, agent_fact_pack
                        ),
                        required_anchors=_agent_interview_anchors(
                            state, agent_fact_pack
                        ),
                        portrait=state.profile,
                    )
                    if (
                        agent_result.status == "success"
                        and agent_result.text
                    ):
                        visible = agent_result.text
                    logger.info(
                        "qxd_agent_turn status=%s tools=%s",
                        agent_result.status,
                        ",".join(agent_result.tool_names) or "none",
                    )
                    # v4.2.0 记住本轮实际展示话术（Agent 路径同样持久化，
                    # 语义与表达层路径一致），下一轮据此防重复承接。
                    set_session_value(
                        db,
                        session_id=session_id,
                        student_id=student_id,
                    ...
                    not state.needs_confirmation
                    and not answered_single_choice
                ):
                ...
                elif answered_single_choice:
                    # v4.2.x 修复5：单选轮跳过 LLM 增强（消除串行等待），但本轮
                    # 实际展示话术仍持久化，保持 v4.2.0 多轮自然度（防重复承接）。
                    set_session_value(
                        db,
                        session_id=session_id,
                        student_id=student_id,
            ...
            if state.recommend_ready:
                ...
                # v4.2.2（P0-J）：推荐后"路由黑洞"治理 —— 结构指令之外的
                # 普通提问先走确定性通用问答回退（复合指令拆分 / 告别 /
                # 导师知识库 / 平台 FAQ / 能力引导），不再无条件重跑匹配。
                if _is_compound_command(latest_user):
                    general_reply = _COMPOUND_CMD_GUIDE
                elif not (
                    _fallback_command(latest_user)
                    or _parse_ordinal(latest_user) is not None
                    or latest_user == _REPORT_DELIVERY_CONFIRMATION
                    or any(
                        term in latest_user
                        for term in (
                            _RADAR_INTENTS
                    ...
                    # v4.3.0 接线点 B（匹配后非结构化消息 Agent 钩子）：
                    # 在所有结构化 handler（二次筛选/序号/雷达/报告/套磁/
                    # 放宽/换方向/结果重跑，以及告别/感谢/问候等廉价确定性
                    # 短回复）之后、确定性兜底链（松散导师问法/FAQ/结果
                    # 指引/能力引导）之前。已被前置结构化 handler 消费的
                    # 消息不会到达这里（确定性保留区）。Agent 失败则继续
                    # 走既有兜底链，行为不变。
                    if not latest_user.strip():
                        general_reply = None
                    else:
                        cheap_reply = _post_match_cheap_reply(latest_user)
                        if cheap_reply is not None:
                            general_reply = cheap_reply
                        elif _post_match_rerun_release(latest_user):
                            general_reply = None
                        elif _agent_hook_active(
                            request, pending_attachments
                        ):
                            post_match_portrait = _safe_confirmed_portrait(
                                db,
                                session_id=session_id,
                                student_id=student_id,
                            )
                            agent_result = await run_agent_turn(
                                ...
                                student_id=student_id,
                                messages=messages,
                                state_context=_agent_post_match_state_context(
                                    post_match_portrait,
                                    get_session_value(
                                        db,
                                        session_id=session_id,
                                ...
                                # compute_match 的适配分数锚点由编排层
                                # 通过 anchor_sink 自动追加校验。
                                required_anchors=[],
                                portrait=post_match_portrait,
                            )
                            logger.info(
                                "qxd_agent_turn status=%s tools=%s",
                                agent_result.status,
                            ...
                            general_reply = _post_match_general_query(
                                latest_user=latest_user,
                                student_id=student_id,
                                db=db,
                            )
                ...
                # v4.2.x 修复8：无候选兜底指令 ——「放宽」忽略硬性条件重跑；
                # 「换方向：XXX」按新主题重跑（单次生效，不改写已确认画像）。
                relax_hard = _relax_trigger_match(latest_user)
                topic_override = _parse_topic_override(latest_user)
                outcome = run_confirmed_match(
                    ...
                    extra_topic_tags=topic_override,
                )
                ...
                # v4.2.x 修复8：放宽/换方向重跑在结果前标注本次口径，诚实可溯
                # （画像本身未被改写，仍保持已确认状态）。
                if relax_hard:
                    visible = f"已按「放宽」忽略硬性条件重新匹配：\n\n{visible}"
                elif topic_override:
                    visible = (
                        "已按「"
                        + "、".join(topic_override)
                ...
                if (
                    ...
                    if (
                        ...
                        elif _fallback_command(latest_user):
                            # v4.2.x 修复8：确认/放宽/换方向触发空结果时，
                            # 展示诚实空态 + 一次性兜底引导（不重复刷屏）。
                            visible = _append_no_candidate_card(
                                db,
                                session_id=session_id,
                                student_id=student_id,
                                visible=visible,
...
async def chat(
    ...
    if not request.stream or not settings.QXD_CHAT_STREAM_ENABLED:
        # v4.2.2（回归问题5）：QXD_CHAT_STREAM_ENABLED=false 时即使请求方
        # 传 stream=true 也按一次性 JSON 返回——绕开清小搭平台转发层间歇性
        # 不消费 SSE 收尾帧导致的 2 分钟假死；协议兼容（客户端按常规 JSON
        # 消费），访谈回复均为短文本，非流式代价可接受。
        return JSONResponse(
            _nonstream_payload(
    ...
    async def sse_stream():
        ...
        # v4.2.x 修复4：收尾帧（finish_reason + data: [DONE]）是"必达"契约，
        # 前端据此解除 loading。content 为空/None 时先兜底为空串，避免在
        # len() 处抛异常导致流中断、收尾帧缺失（网关已 proxy_buffering off，
        # 服务端不再有缓存层吞帧）。
        ...
        for index in range(0, len(content), 8):
            yield _stream_frame(
                ...
                created=created,
                model=model,
                delta={"content": content[index : index + 8]},
            )
            await asyncio.sleep(0)
        yield _stream_frame(
            completion_id=completion_id,

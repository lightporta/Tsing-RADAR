...
UTTERANCE_DIMENSION_MAP: tuple[tuple[str, str], ...] = (
    ...
    # v4.2.2 P2-Q：裸"产出/效率"（"很在意经费和产出效率"）
    ("产出", DIMENSION_EFFICIENCY),
    ("效率", DIMENSION_EFFICIENCY),
    # 指导意愿
    ...
    # v4.2.2 P2-Q：裸"经费"（"很在意经费和产出效率"）
    ("经费", DIMENSION_FUNDING),
    # 人脉资源
...
def detect_implicit_dimension_attention(
    ...
    *,
    ...
    v4.2.2（P2-Q）：单条消息同时命中 ≥2 个不同维度（如"很在意经费和
    产出效率"）即为强关注证据，按命中 2 次计重；跨轮同一维度命中 ≥2 次
    照旧计入。返回按证据强度降序的维度列表。
    """
    counts: dict[str, int] = {}
    for message in user_messages[-window:]:
        dimensions = map_utterance_dimensions(message)
        # 一条消息点出多个维度 → 显式关注，计 2 次权重；
        # 单维度单次命中仍不视为关注（避免口语被过度解读）。
        weight = 2 if len(dimensions) >= 2 else 1
        for dimension in dimensions:
            counts[dimension] = counts.get(dimension, 0) + weight
    return sorted(
...
# 套磁邮件触发词（v4.2.2 P1-O 收紧：仅显式"写/发邮件、套磁"类动词触发。
# 删除"联系导师/怎么联系"——告别与普通陈述（"我去联系导师了"）会被误判
# 为邮件意图；"怎么联系导师"改由 FAQ 词表承接（官方渠道指引，不生成邮件）。
# 保留"发简历/简历发给"：明确"把简历发给导师"动作，属套磁工作流。）

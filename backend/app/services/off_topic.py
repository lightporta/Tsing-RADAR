...
# P2-E 修复：英文方向词（词边界匹配，命中即视为科研相关，不被判跑题）。
# 覆盖常见缩写与学科名；只放行、不映射（标签按原词入档，词法召回可命中）。
_ENGLISH_DIRECTION_WORDS: tuple[str, ...] = (
    "nlp", "llm", "cv", "ml", "dl", "ai", "robotics", "robot", "system",
    "network", "database", "security", "vision", "language", "learning",
    "deep learning", "machine learning", "natural language", "speech",
    "audio", "graphics", "hci", "architecture", "compiler", "distributed",
    "cloud", "quantum", "crypto", "blockchain", "optimization", "algorithm",
    "theory", "data mining", "data science", "embedded", "iot", "chip",
    "semiconductor", "bioinformatics", "mathematics", "physics", "chemistry",
    "biology", "statistics", "finance", "remote sensing", "satellite",
)


def _contains_english_direction(text: str) -> bool:
    """词边界命中英文方向词（大小写不敏感）。"""
    lowered = f" {text.lower()} "
    return any(
        f" {word} " in lowered or lowered.startswith(f"{word} ") or lowered.endswith(f" {word}")
        for word in _ENGLISH_DIRECTION_WORDS
    )
...
CONSTRAINT_NEGATIVE_RE = re.compile(
    ...
    r"|(?:不是|不算|并非|不构成|谈不上).{0,4}(?:硬)?(?:约束|条件|限制)"
)
...
# v4.2.2（P0-J/P1-O）：告别词。告别消息应得到简短回应+引导，绝不触发
# 套磁邮件生成（"谢谢，再见"曾被误路由）或静默重跑匹配。
_FAREWELL_WORDS: tuple[str, ...] = (
    "再见", "拜拜", "告辞", "再会", "晚安", "回见", "下次聊",
    "先走了", "不聊了", "去忙了", "88", "bye",
)


def is_farewell(text: str) -> bool:
    return contains_any(text.strip().lower(), _FAREWELL_WORDS)
...
def detect_off_topic_interests(answer: str) -> bool:
    ...
    if contains_any(lowered, _direction_words() + _RESEARCH_WORDS):
        return False
    if _contains_english_direction(text):
        return False  # P2-E：英文方向表达（I'm interested in NLP）不算跑题

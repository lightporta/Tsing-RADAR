...
_PLATFORM_FAQ: tuple[tuple[str, str], ...] = (
    ...
    (
        # v4.2.2（P0-J）：黑洞态通用问答回退 —— 算法类提问不再吞进重匹配
        "契合度",
        "契合度分数来自画像六维偏好与导师已审核公开证据的确定性映射："
        "类别命中、混合桥接（mixed/balanced）等规则逐项打分后加权汇总，"
        "满分 100。分数只反映规则匹配程度，不构成对导师水平的评判；"
        "门禁未开放的维度不会用推断值冒充。",
    ),
    (
        # v4.2.2（P0-J）：算法/来源类提问的确定性答案（与"契合度"同批）
        "分数怎么算",
        "分数怎么算：每一维先判断「用户偏好 ↔ 导师已审核证据」的类别"
        "命中（完全一致 1.0、混合桥接 0.6~0.7、不一致 0），再按维度权重"
        "加权。规则与权重见网站「匹配怎么算」说明，聊天内只展示结果口径，"
        "不对外暴露可被刷分的内部细节。",
    ),
    (
        # v4.2.2（P0-J）：数据可信度提问的诚实答复（不编造来源）
        "数据从哪",
        "数据来源：导师信息全部来自公开可查来源（官网主页、公开成果、"
        "匿名评价综述的聚合结果），每条证据带来源与审核标记；聊天内绝不"
        "编造导师方向、成果或名额。具体来源与更新口径见网站「数据来源」"
        "页面。",
    ),
    (
        "数据来源",
        "同上：所有评分与结论都基于公开数据与已审核证据，未经审核的推断"
        "不会冒充事实。导师个体信息（组会/延毕/招生名额）知识库未收录时"
        "会明确告知，建议以官网和官方邮箱为准。",
    ),
    (
        "可信",
        "可信度红线：① 只使用公开可查来源与已审核证据；② 知识库未收录的"
        "个体信息明确说「未收录」，绝不编造；③ 评分门禁未开放时不输出推断"
        "分数。你可以随时追问任何一条结论的来源依据。",
    ),
    (
        "如何开始",
...
# v4.2.2（P1-O）："怎么联系导师/老师"走 FAQ（官方渠道指引），绝不生成邮件。
_CONTACT_MENTOR_TERMS = (
    "怎么联系导师",
    "怎么联系老师",
    "如何联系导师",
    "如何联系老师",
...
_CONTACT_MENTOR_ANSWER = (
    "联系导师请以官方渠道为准：\n"
    "1. 在导师主页（院系官网师资页）查看官方邮箱与办公室；\n"
    "2. 联系院系研究生教务办公室核实招生与联系方式；\n"
    "3. 平台不代发私信，也不会提供未经核实的私人联系方式。\n"
    "需要的话，我可以帮你起草一封礼貌的套磁邮件（回复「帮我写邮件」即可）。"
)
...
_EMAIL_SYSTEM_PROMPT = (
    ...
    "用「X老师」占位。\n"
    "v4.2.2 硬性要求：正文与主题中禁止出现任何方括号占位符（如"
    "[用户画像中的研究兴趣]、[用户研究兴趣]）或「占位符」字样；"
    "研究兴趣缺失时如实写「我的研究方向尚未整理好，希望能进一步了解贵组"
    "的课题安排」，绝不替用户编造或留方括号空位。"
)
...
def _interests_text(portrait: StudentPortrait | None) -> str:
    """研究兴趣文本；v4.2.2 P1-O：空时返回"（未提供）"而非"请用占位符"。

    旧文案会把"占位符"指令透传给 LLM，诱使它输出
    "[用户画像中的研究兴趣]"之类的假值；现改为中性表达，
    并在 _llm_email_draft 对含占位符字样的输出做拒绝（fail-closed 降级）。
    """
    interests = list(getattr(portrait, "research_interests", None) or [])
    return "、".join(interests) if interests else "（未提供）"
...
def deterministic_email_draft(
    *,
    ...
    # v4.2.2 P1-O：画像无兴趣时不写"（未提供）方向"这种病句，
    # 用"相关方向"承接；列表项如实标注未提供（发送前自行补充）。
    has_interests = interests != "（未提供）"
    interest_phrase = f"{interests}方向" if has_interests else "相关方向"
    subject = f"关于加入{advisor}课题组的研究申请"
    lines = [
        f"{advisor}，您好！",
        "",
        f"我是清华大学的一名学生，对{interest_phrase}有持续的兴趣，"
        "看到您在相关领域的工作，希望能进一步了解您课题组的研究安排。",
        "",
        "【我的基本情况】",
        "- 研究兴趣：{}".format(interests if has_interests else "（未提供，发送前请补充）"),
...
async def _llm_email_draft(
    *,
    ...
    if not subject or not body or len(body) > 3000:
        return None
    # v4.2.2 P1-O：含方括号占位符（"[用户画像中的研究兴趣]"等）或
    # 占位符字样/待补提示的输出视为污染，拒绝并降级确定性模板。
    placeholder_markers = ("[", "]", "占位符", "待补充", "待填写")
...
async def handle_consult_faq(
    *,
    ...
    for topic, answer in _PLATFORM_FAQ:
        if topic in text:
            return answer, None
    if any(term in text for term in _CONTACT_MENTOR_TERMS):
        # v4.2.2（P1-O）：官方渠道指引，绝不生成邮件/编造联系方式
        return _CONTACT_MENTOR_ANSWER, None

...
# 09 文档 L3：3 → 5（glm-5-turbo 指令遵循更强，单轮允许多查多想）
_MAX_TOOL_CALLS_PER_TURN = 5
# 最终回复长度上限（超长 → rejected_by_gate）
_MAX_REPLY_CHARS = 2000
# 单轮编排总预算（秒）：覆盖该轮全部 GLM 调用与工具执行
# 09 文档 L3：75 → 90（工具调用上限 3→5 提高后，单轮预算同步放宽，
# 防止多一轮工具循环被总预算误杀拖成 budget_exhausted）
_TURN_TOTAL_BUDGET_SECONDS = 90.0
# GLM 采样参数：09 文档 L3 由 0.3 放开到 0.7——表达更自然（红线：
# 温度上限 0.8，超过即开始漂事实，禁止再调高；事实与分数仍由
# 确定性工具 + 逐字锚点校验兜底，温度只影响措辞不影响数值契约）
_TEMPERATURE = 0.7
_MAX_TOKENS = 800
# 送入 GLM 的历史消息条数上限（仅 user/assistant 角色）
# 09 文档 L3：12 → 20（拉长上下文记忆，多轮访谈的呼应更自然）
_HISTORY_LIMIT = 20
...
def _build_agent_messages(
    ...
    # 09 文档 L2 锚点收缩：题干不再要求逐字——可自由改写包装（问出同一
    # 问题即可），全部选项原文仍逐字锚定（红线不动）。
    system_content = (
        AGENT_SYSTEM_PROMPT
        + "\n\n【服务端确定性状态——最高优先级指令，效力高于本提示词前文的一切工作流描述与示例】\n"
        + "当前轮次必须以下方状态为准：若下方已下发当前题目与选项，"
        "必须完整呈现全部选项原文（逐字，编号选项不计入字数限制），"
        "题干可自由改写包装但必须问出同一问题、不得跳过当前题目，"

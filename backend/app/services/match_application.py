...
def run_confirmed_match(
    ...
    *,
    ...
    if not result.items:
        ...
        if zero_reason:
            # v4.2.2：约束归零 / 召回归零的原因原文（matching 层已区分），
            # 放宽后归零不再出现"硬约束"误导文案。
            message = zero_reason
        elif relax_hard_constraints:
            # 防御兜底：已忽略硬性条件仍归零且无具体原因 → 明确与约束无关
            message = (
                "已忽略硬性条件后仍没有候选：归零原因是画像研究方向与导师"
                "目录重合不足（召回归零），与硬约束无关。可以回复"
                "「换方向：XXX」换主题重试。"
            )
        else:
            message = (
        ...
        return MatchApplicationOutcome(
            ...
            questions=[
                ...
                if zero_reason and "召回" not in zero_reason
                else "可以回复「换方向：XXX」按新主题重试。"
            ],
        )
    return MatchApplicationOutcome(
        status="matched",

...
的确定性投影），配套跨轮防重复闸门与提示词 v4。红线不变：确认门与
匹配结果不增强；选项/题干/招募/记忆逐字校验照常生效（v4.2.2 调和：
选项题以全部选项逐字为唯一内容锚点，自由文本题保留题干 ≥5 字片段，
与模板 v4 的逐字指令一致，消除 v3 意译指令与校验器的自相矛盾）。
"""
...
# prompt_versions.json 登记的当前版本加载（v4.2.0 起引入多轮上下文，
# v4.2.2 为 v4：调和选项逐字/题干片段规则与校验器一致）；加载失败 →
# 回退本常量（fail-closed）。
...
def _core_fragments(prompt: str) -> list[str]:
    """取题目中长度 >=5 的片段作为覆盖检查依据（宽松匹配）。

    v4.2.2：6 → 5。真实题库自由文本题的题干常按 5 字碎成片段
    （如「对硬性条件」），6 字下限会让校验器放行缺失题干核心的
    输出；测试锚点（4 字「喜欢什么」不产生片段）不受影响。
    """
    return [
        part.strip()
...
def _validate_expression(text: str, fact_pack: InterviewFactPack) -> bool:
    ...
    if fact_pack.options and not all(
        ...
        return False
    # v4.2.2（GLM 真实 key 验证）：选项题以「全部选项逐字」为唯一内容锚点。
    # v3 模板自然度 C 曾指令「选项意译」，而校验器同时要求题干 ≥6 字片段，
    # 真实 GLM-4-Flash 意译后永远无法满足 → 增强 100% 被拒。v4 模板改为
    # 要求逐字引用全部选项后，选项逐字检查严格强于题面片段检查，片段检查
    # 反而会误伤（如 GLM 逐字引用了全部选项却改了题面措辞）。
    # 自由文本题（无选项）保留题干 ≥5 字片段作为内容锚点。
    if not fact_pack.options:

...
def test_single_choice_turn_skips_llm_enhancement(monkeypatch):
    ...
    # 旧表达层用例：关闭 v4.3.0 全 Agent 化编排（Agent 路径下单选轮由
    # 编排层接管，不经 render_interview_reply），保持旧路径覆盖。
    monkeypatch.setattr(chat_module.settings, "AGENT_ORCHESTRATION_ENABLED", False)
    calls: list[str] = []

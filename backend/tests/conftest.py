...
# 测试默认禁用本机 backend/.env 加载（开发机真实密钥不得泄漏进测试构造的
# Settings，如 test_l1_production 的生产配置用例），并与下面的凭据中和双保险。
os.environ["TSING_RADAR_ENV_FILE"] = ""

# 测试默认中和 LLM 凭据（空 key → credentials=()，LLM 路径全部走确定性降级），
# 避免本机 backend/.env 的真实 GLM key 污染测试（真实网络调用 + 不确定输出）；
# 需要 LLM 的测试用 monkeypatch 显式注入假凭据 + mock HTTP（test_agent_orchestrator）。
# 注意：LLM_PROVIDER 是 Literal["glm"]，不能置空串，不在此覆盖。
os.environ["GLM_API_KEY"] = ""
os.environ["LLM_API_KEY_FILE"] = ""
...
# 仓库根（Tsing-RADAR/）也纳入 sys.path：scripts 为命名空间包（无
# __init__.py），合并后 test_l1_production 等用例可导入仓库根 scripts/
# 下的 L1/L2/L3 检查脚本，backend/scripts 下的脚本导入不受影响。

...
def _write_report(
    ...
    lines += [
        "",
        "## 五、提示词版本化记录（任务书 A-3；v4.1.0 升级 v2）",
        "",
        "| 模板 | 版本 | 变更 |",
        "|---|---|---|",
        "| system_prompt | v2 | 学长/学姐人设 + 务实真诚基调 + 客服腔/AI 自称/空洞鼓励禁令；保留 v1 状态机控制规则 |",
        "| rewrite_template | v2 | 六条自然度要求（承接轮换/选项融句/禁机器腔开头结尾/语气词适度）；保留 v1 全部事实红线 |",
        "",
        "版本文件：`backend/app/services/prompts/`（*_v1.txt / *_v2.txt + prompt_versions.json）；",
        "加载失败回退内嵌 v1 兜底（fail-closed）。v2 配套表达层自然度闸门",
        "（`chat_expression._NATURALNESS_REJECT_TOKENS`）：输出含机器腔/客服腔标记",
        "即拒绝并降级固定模板——自然度违规的处理方式与事实违规一致（宁降级不出戏）。",
        "",
        "## 六、工具注册表记录（任务书 阶段B）",
        "",

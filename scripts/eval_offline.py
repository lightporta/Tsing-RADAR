...
def _write_report(
    ...
    lines += [
        ...
        "| 模板 | 版本 | 变更 |",
        "|---|---|---|",
        "| system_prompt | v1 | 与 v3.1.x 内嵌文本逐字一致；版本化加载 + 内嵌兜底 |",
        "| rewrite_template | v1 | 与 v3.1.x 内嵌文本逐字一致（含招募/记忆事实段逐字保留要求） |",
        "",
        "版本文件：`backend/app/services/prompts/`（*_v1.txt + prompt_versions.json）；",
        "加载失败回退内嵌 v1 兜底（fail-closed），行为与 v3.1.x 完全一致。",
        "",
        "## 六、工具注册表记录（任务书 阶段B）",
        "",

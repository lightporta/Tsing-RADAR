...
> 按《问题清单_统一文档》+《v4.2.1线上回归报告》修复全部待修项，并用真实
> GLM API key 完成端到端真实验证（此前团队无 key，LLM 增强链路从未被真实验证）。
> 全量门禁：**882 passed / 4 failed（同一 Windows 环境性基线）/ 2 skipped /
> 2 errors —— 0 新增失败**（新增 v4.2.2 回归用例 26 例 + 表达层调和用例 5 例，
> 见 `backend/tests/test_fixes_v422.py` 与 `test_chat_expression.py::TestV422TemplateHarmonization`）。

### 修复（回归报告 问题1-5 + 问题清单 P0/P1/P2 待修项）

- **回归问题1+2 / P0-A 画像中途损坏**：单选维度句子式答案走 LLM 慢路径失败后丢弃 →
  回退环/跳问/字段错位。修复：四个单选维度（研究方式/指导偏好/生涯方向/创新风险）
  增加确定性关键词降级映射（interview.py 权威词表），句子答案就近映射到选项值而非
  丢弃；研究兴趣无方向词时熔断（`_INTEREST_DECLINE`），不再写入空兴趣。
- **回归问题3+4 / P0-B 残留**：「放宽」真正传递到匹配层（relax_hard_constraints
  生效），重跑不再误报硬约束归零；兜底卡每会话仅一次（`no_candidate_card_shown`）；
  空态文案区分「约束归零」vs「召回归零」；硬约束重复登记去重（同值确认只入档一次）。
- **回归问题5 / P1-C 响应波动 + 假死**：`QXD_CHAT_STREAM_ENABLED=False` 时
  stream=True 请求返回非 SSE JSON（绕开平台转发层不消费 SSE 收尾帧导致的假死）；
  单选轮跳过 LLM 增强（修复5，串行等待归零）。
- **P0-J 推荐后路由黑洞**：`_post_match_general_query` 确定性通用问答回退——告别 /
  致谢 / 问候 / 松散导师查询 / 平台 FAQ / 结果指引 / 能力引导；复合指令拆分逐条路由；
  显式「查看/结果/匹配」追问与匹配相关提问放行走既有流程；「确认生成并通过清小搭
  附件交付」放行到附件签发分支（附件与 503 前置校验恢复触发）。
- **P1-N 多分句边界**：逐条审问改为只审问真正含约束的分句；否认（「不是约束」「无」）
  不再被引用成约束值。
- **P1-K off_topic 误杀**：选项精确匹配前置（「放手探索」等选项原文不再被判跑题）；
  「无」等逃生词放行。
- **P1-D 跑题闲聊编造**：已匹配态非指令消息一律走能力引导 / 诚实拒答，绝不编造。
- **P1-O 套磁邮件**：仅显式「写/套磁」动词触发；画像实际值注入，占位符不再泄漏。
- **P1-H/P1-I 画像长句/开场残留**：连接词碎块清洗、多维修改不再残余混入画像。
- **P2-Q 维度权重对话入口**：「在意/优先/重要+维度」表达识别并写入偏好补充字段。
- **P2-E 英文输入**：英文兴趣句提取方向标签并推进状态机（单语解析补充）。

### 增强链路调和（真实 key 验证发现并修复）

- **根因**：v3 模板自然度 C 指令「选项意译」，而校验器要求全部选项逐字 + 题干
  ≥6 字片段 —— 自相矛盾，真实 GLM-4-Flash 输出永远被拒（增强 100% 降级且从未被
  发现，团队无 key）。
- **修复**：新增 `rewrite_template_v4.txt`（有选项时逐字引用全部选项、无选项时保留
  题干 ≥5 字原句片段）；校验器同步——选项题以全部选项逐字为唯一内容锚点、自由文本题
  保留题干 ≥5 字片段检查（`_core_fragments` 6→5）；`prompt_versions.json` 与
  `_CURRENT_VERSIONS` 锁步升级 v4（版本不一致即兜底 v1，fail-closed）。
- **真实验证（GLM-4-Flash）**：长标签单选轮与自由文本轮均无法逐字引用 → 按设计
  降级确定性模板（宁降级不出戏），回复确定且快速；矛盾已消除，后续模型可逐字引用
  时增强自动生效。降级成本有界：仅兴趣→研究方式一轮 1 次调用（≤4s），单选轮零调用。

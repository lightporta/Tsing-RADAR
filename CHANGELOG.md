...
> 按《Tsing-RADAR 对话增强适配执行提示词_20260822》四阶段全量交付
> （任务书原定版本号 v4.2.0 已被多轮自然度批次占用，本批顺延 v4.3.0；
> 分支 `feature/dialogue-enhance-v4.3`，每阶段独立 commit）。核心张力
> 与解法：AI 自然度 vs 确定性主干——**不扩大 LLM 权限，只扩大其可见的
> 确定性事实**；工具域转正是唯一破例（白名单制 + 测试锁死）。

### 新增

- **阶段一 · RADAR娘人设 v4 + 三明治闲聊容忍**（`0fe7232`）：
  - 提示词 v4（版本纪律：新增文件不覆盖；任务书原定 v3 已被占用）——
    RADAR娘人设（选师+避坑经验、网络语气、单 emoji、3~5 句）、三明治
    法则（共情≤30% → 四类钩子至少选一 → 回归主线）、首句承接上轮、
    隐性提及记忆事实、纯文本输出禁 Markdown；
  - 轻闲聊三明治 nudge（`off_topic.is_light_chitchat`，与硬红线词表
    严格互斥）：共情 + 不写入画像 + 回题；会话级 ≤5 轮（第 6 轮起回
    能力引导）；防吸收守卫回归锁定；
  - 敏感话题（外置词表 `CHAT_SENSITIVE_WORDS`，默认空不拦截）：命中
    明确拒绝并回主线；
  - 表达层 Markdown 闸门（`**`/标题/代码块/行首列表 → 拒绝降级）。
- **阶段三 · 向量混合召回（零依赖，诚实降级）**（`7876346`）：
  - `llm.embed_text_strict`（无 key/失败返回 None，绝不落 hash 兜底）；
  - `mentor_knowledge_vector`：纯 Python 余弦（无 numpy），阈值门控
    （`KNOWLEDGE_VECTOR_MIN_SIMILARITY=0.60`）守住拒答门；三层降级
    链与词法基线逐字一致；确定性排序（相似度降序 + 同分姓名序）；
  - `build_mentor_knowledge.py --rebuild-vectors`（人工触发）：无 key /
    嵌入失败 / 维度不一致诚实退出不产半成品；manifest 增补
    vectors_sha256 溯源；语义补充渲染明示「未收录 + 语义相近」。
- **阶段二 · 沟通阶段记忆键**（`b457293`）：`communication_stage`
  枚举（初选/联系中/已约谈）+ **确定性事件触发**（匹配展示→初选、
  套磁生成→联系中、站内投递→已约谈）；`remember_communication_stage`
  唯一写入口，非枚举值抛 ValueError——LLM/用户自由文本结构上无法写库；
  只前进不回退；无迁移。
- **阶段五 · LLM 自主工具调用（工具域转正）**（`55a856a`）：
  - 协议：`_llm_complete_result` 支持 tools 参数与 tool_calls 解析
    （畸形 JSON → None fail-closed；截断 ≤3）；
  - `autonomous_tools.try_autonomous_tool_call`：仅匹配态兜底位触发；
    无 key/开关关/失败/无调用 → 降级行为与基线逐字一致；
  - **收藏**：`mentor_favorites` 表 + 迁移 0015；幂等；advisor_id 只能
    来自当前匹配上下文（防幻觉 ID）；意图词路由（收藏第 N 个/收藏
    姓名/我的收藏/取消收藏）与自主调用双路径同经 dispatch 校验执行；
  - **send_contact_request 强制二次确认**（反骚扰）：执行体只登记
    待确认动作并返回精确确认指令；确认门先于意图分类；非确认/取消
    消息即失效；确认后走既有套磁链路 + 沟通阶段「联系中」；
  - **注册表白名单制**：`llm_callable`/`sensitive` 显式标记；
    画像确认/匹配触发/记忆写入/招募发布**永不注册**（架构护栏断言）；
    幻觉工具名/参数注入 → 确定性拒绝文本。
- **评估闭环扩容**：`eval_offline.py` 60 → **75 例**（新增 v43_chitchat 5 /
  sensitive 2 / persona 2 / vector 3 / tools 3 分组；用例级
  `env_patches` 注入敏感词表/匹配上下文/模拟 LLM tool_calls）；
  **对话质量周检流程** `docs/对话质量周检记录.md` 首版入库（抽样
  口径/失败模式分类→处置映射/回归验证规则）。

### 文档

- 偏差修正记录表 §五：mem0 / LLM 意图分类 / get_mentor_availability /
  目标院校拒绝理由；schedule_reminder 缓期（需到期触发机制，与无后台
  调度架构不匹配）；自主调用转正边界（白名单制）；
- RUNBOOK：向量工件（随 release 分发/缺失降级/与知识本体同步重建）+
  迁移链 0013→0015（expand-only）；RESOURCE_BUDGET：向量/收藏/工具
  组件资源足迹（零新增服务）；README/技术文档表数 29→30。

### 验证

- 后端全量：**864 passed / 1 failed**（docker 环境项不变；本轮 +71
  用例：阶段一 14 + 阶段三 19 + 阶段二 9 + 阶段五 26 + 评估谓词）；
- 离线评估：**75/75 通过、红线违规 0/17、事实保真 5/5**；
- 前端 `vue-tsc --noEmit` + `vite build` 通过（本轮前端零改动）。

### 红线（逐字不变）

确认门与匹配结果不进表达层；题面/选项/招募/记忆逐字校验；无凭据/
失败完全降级固定模板；诚实空态；写入门禁（记忆白名单 + 枚举值域 +
工具域白名单）；匹配/确认门永不由 LLM 决策。

---

## v4.2.1（2026-08-21）

> 按《访谈引擎修复方案》落地 8 项修复（真实负载压测：27 场对话、265+ 轮，
> 幽灵硬约束 21 例/9 例无候选、边界澄清死循环 ~50%、跑题回声环 ~30%）。
> 完整映射与验收口径见 `docs/访谈引擎修复方案验收记录_v421.md`；验收测试
> `backend/tests/test_interview_fixes_v421.py` 19 例全绿。

### 修复

- **P0-1 幽灵硬约束污染匹配**（三层消毒）：① 边界澄清环「确认画像 / 态度词 /
  开场白残留」命中黑名单 → 整体清空，绝不生成草案（`off_topic.CONSTRAINT_JUNK_SIGNALS`
  + `CONSTRAINT_NEGATIVE_RE` 收口为权威词表，interview / matching 共用一份）；
  ② 负向整句（「没有硬约束，所有条件都只是一般偏好」等）走清空分支不入档；
  ③ 匹配前 `parse_hard_constraints` 第三层丢弃幽灵值；`run_confirmed_match`
  对 >3 条硬约束按污染整批忽略（记日志）。
- **P0-2 边界澄清死循环**：负向/无类答案一次即闭合边界直出画像卡；「确认」遇无
  结构化草案 → 放弃草案不再原地循环；同一澄清提问连续 3 次无进展 → 强制闭合
  边界并明示「先不再追问，其余按无硬性条件处理」。
- **P1-3 跑题回声环**：边界澄清环「确认画像」等确认指令短路闭合边界（已确认约束
  来源白名单保留）；off_topic 守卫对负向/态度式回答放行（不再误判跑题重问）。
- **P1-4 前端假死（SSE 收尾必达）**：流式回复全程以 `finish_reason=stop`（嵌套于
  choices[0]）+ `data: [DONE]` 收尾；`sse_stream` 空 content 帧安全。v4.2.0 已具备
  [DONE] 与 4s 增强超时，本轮补内容守卫与契约断言。
- **P1-5 增强串行延迟降级**：单选轮（研究方式/指导偏好/生涯方向/创新风险）跳过
  LLM 增强 → 单轮零 GLM 等待（验收 ≤10s）；自由文本轮（研究兴趣/硬约束）保留
  增强（≤4s 超时、逐字校验红线不变）；被跳过轮仍持久化实际展示话术，保持
  v4.2.0 多轮自然度（防重复承接）语义。
- **P2-6 画像字段污染**：兴趣标签清洗——前缀（我对/我是/我想做/研究…）、后缀
  （很感兴趣/相关研究…）剥离；命令/提问整句丢弃；单标签 ≤12 字、总量 ≤8；
  「确认画像」等指令绝不写入画像。
- **P2-7 访谈流程游走**：每维度恰好问一次、顺序固定（QUESTION_ORDER），已答维度
  不再重复问/跳问（以验收用例锁定既有状态机语义）。
- **P2-8 无候选后无引导**：空结果（matched 但 0 候选）追加一次性兜底卡——回复
  「放宽」忽略硬性条件重跑 / 「换方向：XXX」按新主题重跑（重跑结果带口径标注）/
  完整导师目录链接；每会话仅出现一次，其后跑题消息给能力引导不再刷屏。

### 验证

- 验收套件 `test_interview_fixes_v421.py` **19/19**；契约/记忆测试随修复5同步更新
  （契约口径与偏差见 `docs/访谈引擎修复方案验收记录_v421.md` 偏差 D-6/D-7）。
- 后端全量：**851 passed / 4 failed（环境性基线不变：wqy 字体、归档字节确定性、
  LLM 密钥符号链接与 POSIX 权限）/ 2 skipped / 2 errors**，0 新增失败
  （基线 832 → +19 验收用例）。
- 红线不变：确认门（needs_confirmation）与匹配结果（recommend_ready）不进表达层；
  无凭据/失败完全降级固定模板；诚实空态与六维门控不放松。

---

## v4.2.0（2026-08-20）

> **表达层多轮自然度增强**（在 v4.1.0 返工批次——提示词 v2 自然度 + 机器腔闸门、
> 招募 FactPack 生产接线、雷达 CJK 对齐等，见提交 228a673/b496796/a33fc48——之上）。
> 核心思路：不放松任何红线，只把表达层可见的**确定性事实上下文**从「最后一句用户
> 输入」扩展为「多轮投影」，让 LLM 重写有据可依、有据可拒。

### 新增

- **FactPack 多轮上下文四字段**（`chat_expression.InterviewFactPack`，全部为服务端
  已有事实的只读确定性投影，无 LLM 参与）：
  - `recent_dialogue`：最近 6 轮「用户：…/清小搭：…」对话底稿（末位本轮话术跳过，
    单轮 120 字 / 总量 800 字截断）——多轮连贯呼应的唯一事实来源；
  - `previous_reply`：上一轮**实际展示话术**（新会话级 KV `interview_last_expression`
    持久化，`chat.py` 每轮回注；无值回退状态机底稿推导）——防重复对齐用户真实所见；
  - `turn_phase`（开场/中段/收尾）与 `user_style_hint`（简短/常规/详细，长度分档）：
    阶段化语气与篇幅镜像的确定性标签。
- **跨轮防重复闸门**（`_repetition_violation`，确定性）：开头 10 字（去空白）与上一轮
  完全相同，或复用上一轮 ≥14 字连续片段且不属本轮合法内容（题面/选项/招募/记忆/用户
  原话豁免）→ 拒绝并降级固定模板——「承接方式换着来」从提示词约束升级为可执行闸门。
- **`rewrite_template` v3**（`prompts/rewrite_template_v3.txt` + 版本清单登记，不原地
  覆盖）：保留 v2 六条自然度要求与 v1 全部事实红线，新增 G（多轮连贯：参考最近对话
  呼应、底稿不得照搬、开场与上一轮明显不同）/ H（篇幅贴合用户与阶段）两条要求及
  四段上下文输入；v1 兜底常量不动（`str.format` 忽略多余实参，fail-closed 兼容）。
- **客服套话词表扩充**：很高兴为您 / 为您服务 / 还有什么可以帮 / 希望以上 /
  祝您生活愉快 → 机器腔闸门拒绝降级。
- `dialogue_state_store` 新增 `get_session_value` / `set_session_value`（与一次性
  标记同一存储约定的会话级字符串 KV）。

### 修复

- **`get_dialogue_mode` "none" 占位行劫持意图分类**（潜在缺陷，被本轮每轮写入激活）：
  KV 落库行 mode="none" 曾使后续轮次意图分类短路成 NONE（招募/导师咨询等对话模式
  永不路由）——现归一化为 None（无活动模式），附回归护栏测试。

### 验证

- 后端全量：**776 passed**（新增 18 用例全绿：多轮投影 6 + 防重复闸门 5 + 客服腔 1 +
  v3 模板渲染/兼容 2 + 契约跨轮接线 1 + 意图劫持护栏 1 + prompts v3 合同更新 2；
  环境性基线不变：test_l1_production 依赖 docker CLI、test_l3_handoff 归档确定性
  依赖 docker）。
- 离线评估：**60/60 通过、红线违规 0/17、事实保真 5/5**（`scripts/eval_offline.py`
  复跑，报告提示词版本记录更新为 v3）。
- 红线不变：确认门（needs_confirmation）与匹配结果（recommend_ready）不进表达层；
  题面/选项/招募/记忆逐字校验照常；无凭据/失败完全降级固定模板。

---

## v4.1.0（2026-08-20，追溯补记）

> v4.0.0 审计返工批次（提交 228a673 / b496796 / a33fc48）。当时未留 CHANGELOG
> 条目，本条目为追溯补记，内容与偏差修正记录表_v4.md 一致。

### 修复与增强

- **雷达文本图 CJK 对齐**（`radar_chart.py`）：数值列按 East Asian Width 补位，
  四行数值列肉眼对齐；`render_radar_bars()` 独立柱状渲染 + 形态选择配置
  （八分档 ▏▎▍▌▋▊▉█），修正偏差表中"已补独立柱状图"的虚报。
- **招募事实句接入生产 FactPack**（`chat.py`）：`recruitment_summary` 生产调用点
  接线（此前字段/校验/单测齐备但生产恒为空，属半空壳）；确认门通过后一次性
  主动触达（不刷屏、无相关静默）。
- **记忆隐私对话入口**（`chat.py` MEMORY_VIEW / MEMORY_CLEAR 意图）：查看/清除
  从"有函数无入口"补齐用户触达路径，README 宣传成立。
- **提示词 v2 自然度增强**：学长/学姐人设 + 客服腔/AI 自称禁令（
  `_NATURALNESS_REJECT_TOKENS` 闸门，命中拒绝降级）+ 承接方式轮换要求。
- **README .env.example 断链修复**：新增 `backend/.env.example`。

### 验证

- 后端全量与专项测试随提交记录（表达层 4 + 招募主动触达 + 记忆隐私入口
  用例全绿）；离线评估 60/60、红线违规 0/17 不变。
...
> 匹配后**二次筛选闭环**（换一批 / 缩小范围 / 恢复完整结果）+ **能力差距分析**（需要补充的知识或技能）+ **候选官方主页链接**，并把六维对比升级为 10 格条形可视化——量化优势继续打透。
...
- **契合度构成分解**（`match_application.py`）：新增 `format_fit_breakdown`，把排序流水线已算好的 `score_breakdown`（score×权重×置信度）倒推成可读块——`▲ 拉高 / ▼ 拉低 / · 中位`（按该维得分与 fit_score 差 ≥±3 分判定）+ 权重 + 得分；画像无证据的维度诚实标注`未计入（画像无该维度证据，确认后生效）`，绝不用基准值冒充；标题声明"由排序分数倒推，与保守排序分同一口径，非新增评分"。接入 `format_match_outcome` 候选头部之后，breakdown 缺失/为空时省略该块（旧数据与既有流程零破坏）。特色定位：**量化 + 可解释 + 证据链**。
...
> 科研风格速测（4 题确定性分类）、研究方向地图（不输出教师名单）、画像确认增强（匹配重点 + 未明确项引导）。

### 新增

- **科研风格速测**（`research_style.py`，新）：轻量确定性风格测试——4 题（研究范围 / 推进方式 / 理论 vs 工程 / 成果偏好），9 种"形态 × 驱动"核心风格规则表分类（问题溯源型 / 理论建构型 / 落地攻坚型 / 方法工程型 / 数据驱动型 / 实证归纳型 等），同答案同结果、可测试可复现。诚实红线：只描述当前更偏好的科研方式，**不判断是否适合科研、不评价能力高低**；结果不回填任何六维导师评分，仅提示可选回填画像 `research_mode`（theory/engineering/mixed，"「确认」后生效"）；多轮状态走 `dialogue_sessions`（mode=research_style），支持「取消」退出、非法答案同题重试，完成后自动清除状态。
